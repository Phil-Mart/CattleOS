...
var CATTLEOS = {
  ...
  RAW_CACHE_LIMIT: 8,
  SAFETY_NOTICE: 'ZIP-based results are approximate and use a ZIP-code centroid unless exact coordinates are supplied. Official zones may cover only part of a county. Verify current restrictions and exact property status with the Texas Animal Health Commission before moving animals.'
};
...
CATTLEOS.HEADERS = {
  ...
  Breeding: ['Breeding_ID', 'Animal_ID', 'Event_Date', 'Event_Type', 'Bull_ID', 'Expected_Calving_Date', 'Notes', 'Source_Record_ID'],
  Calves: ['Calf_ID', 'Dam_ID', 'Birth_Date', 'Sex', 'Birth_Weight', 'Status', 'Notes', 'Source_Record_ID'],
  Measurements: ['Measurement_ID', 'Animal_ID', 'Measurement_Date', 'Weight_Lb', 'Body_Condition', 'Notes', 'Source_Record_ID'],
  ...
  Expenses: ['Expense_ID', 'Expense_Date', 'Category', 'Amount', 'Vendor', 'Notes', 'Source_Record_ID'],
  Sales_Harvest: ['Sale_ID', 'Sale_Date', 'Animal_ID', 'Buyer', 'Amount', 'Notes', 'Source_Record_ID'],
  Feed_Forage: ['Feed_ID', 'Record_Date', 'Pasture', 'Feed_Type', 'Quantity', 'Water_Status', 'Concern', 'Source_Record_ID', 'Data_Mode'],
  Pastures: ['Pasture_ID', 'Pasture_Name', 'Acres', 'Fly_Pressure', 'Water_Source', 'Notes', 'Source_Record_ID'],
  ...
  Ranch_Brief_Log: ['Brief_ID', 'Generated_At', 'Generated_By', 'Model', 'Mode', 'Input_Record_Count', 'Input_Hash', 'Output_JSON', 'Rendered_Brief', 'Validation_Status', 'Error_Message']
};
...
function cattle_buildMenu_() {
  ...
  var briefMenu = ui.createMenu('Ranch Brief')
    .addItem('Generate GPT-5.6 Ranch Brief', 'brief_generateRanchBrief')
...
function cattle_setHeaders_(sheet, headers) {
  ...
  var width = Math.max(headers.length, sheet.getMaxColumns());
...
function cattle_openUrlDialog_(title, url) {
  ...
  var html = HtmlService.createHtmlOutput(
    ...
    '<p><a href="' + safeUrl.replace(/"/g, '&quot;') + '" target="_blank" rel="noopener">Open official source</a></p>' +
    '<script>window.open(' + JSON.stringify(safeUrl) + ', "_blank", "noopener");</script>' +
    '</body></html>'
